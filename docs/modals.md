...
| Deleting a recurring time component deletes its events | Nothing in the schema cascades, so the repository deletes events, then slots, then the component. `Event.recurringTimeComponentId` is nullable now — a standalone event has no rule behind it — so the old justification ("an event has no valid state without its component") no longer holds and promoting an exception to standalone is representable. The cascade is kept because an exception to a cadence that no longer exists has nothing to override. Slots keep their ids across an edit precisely so surviving events are not stranded. |
